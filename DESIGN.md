...
- 일반 편집기에서 ANSI 키보드로 수정키 없이 또는 Shift로 입력하는 모든 표준 특수문자는 앞 토큰을 끝내는 보존 구분자로 취급합니다. `_`, `-`, 괄호, 따옴표, Shift 숫자열 기호와 macOS가 치환한 스마트 큰따옴표·길메표도 포함하며 구분자 자체는 바꾸지 않습니다. Command·Control·Option 단축키는 텍스트 경계로 해석하지 않습니다.
- 큰따옴표와 자동 짝닫기처럼 편집기 반영이 늦는 구두점은 최대 약 100ms의 제한된 재확인 창에서 기다립니다. 그 사이 포커스·입력 소스·선택 범위가 달라지면 교정하지 않습니다.
- `.`, `/`, `\\`, `@`처럼 URL·이메일·경로를 이어갈 수 있는 문자는 같은 재확인 창 동안 다음 물리 입력이 없는 경우에만 문장 끝 경계로 허용합니다. 새 이벤트 세대가 보이면 AX 텍스트를 읽거나 바꾸기 전에 복합 식별자로 간주해 실패 폐쇄합니다. `?`는 기존 자연문장 확인 창을 사용합니다.
- 이미 토큰 안에 포함된 snake_case, kebab-case, 경로, URL, 이메일, 코드형 입력을 통째로 변환하지 않습니다.

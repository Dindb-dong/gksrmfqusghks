...
- Space가 AX caret/text에 늦게 반영되는 터미널과 웹 편집기를 위해 최대 4회의 짧은 bounded retry를 허용합니다. 대기 중 새 키, 포커스·입력 소스·보안 상태 변경이나 Space 한 글자로 설명할 수 없는 caret 이동이 생기면 교정하지 않습니다.

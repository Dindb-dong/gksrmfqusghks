...
Space 키 이벤트보다 편집기의 AX text 반영이 늦을 수 있으므로 표준 교정 트랜잭션은 최대 4회의 짧고 제한된 재시도 동안 실제 Space 경계를 기다립니다. 터미널은 한 번의 짧은 커밋 대기 뒤 caret을 연속 조회해 즉시 안정성을 확인하고, AX caret이 아직 게시되지 않은 경우에만 최대 4회 재시도합니다. 각 시도에서 PID·포커스·selection·입력 소스·Secure Input·이벤트 세대를 다시 확인하며, Space 커밋으로 설명 가능한 단 한 번의 `+1` caret 이동은 한 차례 더 즉시 확인하고 그 밖의 변화는 취소합니다. 유효한 caret을 얻은 뒤 남은 재시도 시간을 강제로 기다리지 않아 터미널 프롬프트 재렌더링이나 후속 입력과 불필요하게 경합하지 않습니다.

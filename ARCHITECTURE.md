...
2. 결정 엔진과 강제 안전 필터는 일반 편집기와 동일하게 적용하고 Space와 별도로 식별된 자연문장 물음표 경계만 허용합니다. 물음표는 짧은 확인 창 동안 다음 물리 입력이 없을 때만 문장 끝으로 간주합니다.
3. 교정 직전에 Secure Input, PID, 포커스 identity, caret selection, 번들 제외, 입력 소스와 마지막 물리 이벤트 세대를 다시 검증합니다.
4. 원문과 검증된 경계를 합성 Backspace로 제거하고 후보와 같은 경계를 PID 대상으로 삽입합니다. 모든 합성 이벤트는 sentinel로 재관찰을 막습니다.
5. 절대 caret을 제공하는 터미널은 교정 후 위치까지 검증하고 삭제 의도 추적을 허용합니다. cmux처럼 `AXSelectedTextRange`를 항상 `0:0`으로 제공하는 터미널은 PID·포커스 identity·고정 selection·입력 소스·이벤트 세대를 교정 직전에 재검증하는 상대 교정만 허용하며, 정확한 좌표가 필요한 삭제 의도 학습은 시작하지 않습니다.
...
Space 키 이벤트보다 편집기의 AX text 반영이 늦을 수 있으므로 표준 교정 트랜잭션은 최대 4회의 짧고 제한된 재시도 동안 실제 Space 경계를 기다립니다. 일반 편집기의 특수문자 경계는 직선·스마트 큰따옴표와 자동 짝닫기 반영 지연을 수용하도록 최대 12회의 짧은 재시도를 허용합니다. `_`, `-`, 괄호, 인용부호, Shift 숫자열 기호를 포함해 수정키 없이 또는 Shift로 입력하는 모든 ANSI 특수문자는 앞 단일 토큰과 함께 보존하고, Command·Control·Option 조합은 기존 무효화 경로를 유지합니다. `.`, `/`, `\\`, `@`는 확인 창 전체에서 다음 물리 입력이 없을 때만 확정하며, 이벤트 세대가 바뀌면 AX 조회와 mutation 전에 취소합니다. 자연문장 물음표도 URL 쿼리의 시작과 구분하기 위해 확인 창 전체를 기다립니다. 이미 완성된 경로·URL·식별자 토큰은 계속 실패 폐쇄합니다. 다만 영문 레이아웃 토큰의 대문자가 Shift 두벌식 키(`Q/W/E/R/T/O/P`)로만 구성되고 변환 결과가 사전에 있는 완성형 한글이면, 후보 판정이 camelCase 안전 차단을 좁게 해제합니다. 터미널은 한 번의 짧은 커밋 대기 뒤 caret을 연속 조회해 즉시 안정성을 확인하고, AX caret이 아직 게시되지 않은 경우에만 최대 4회 재시도합니다. 각 시도에서 PID·포커스·selection·입력 소스·Secure Input·이벤트 세대를 다시 확인하며, Space 커밋으로 설명 가능한 단 한 번의 `+1` caret 이동 외의 변화가 있으면 취소합니다.

1. 이벤트 시점의 앱 PID, 포커스 element, selection range를 기록합니다.
2. 실행 직전에 동일 포커스와 예상 caret인지 재확인합니다.
3. 일반 편집기와 절대 caret 터미널은 `AXSelectedTextRange`로 대상 범위를 지정합니다. 명시적 터미널 모드의 고정 `0:0` caret은 상대 합성 이벤트에만 사용합니다.
4. sentinel이 붙은 Unicode 합성 이벤트로 replacement를 입력합니다.
5. 절대 caret은 예상 위치 변화, opaque caret은 동일 PID·포커스와 고정 selection 유지가 일치하면 성공으로 간주합니다.

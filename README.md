...
- Space, ANSI 키보드의 모든 특수문자, 직선·스마트 따옴표와 자연문장 물음표 경계에서의 고신뢰 자동 교정
...
- 전체·단어별 자동 교정 횟수를 보여주는 로컬 통계 탭과 통계 초기화
- Sparkle 기반 자동 업데이트 확인과 수동 `지금 업데이트 확인`

## 개인정보와 안전 원칙

- 키 입력, 판정 중인 현재 단어, 선택 텍스트, 주변 문장은 네트워크로 전송하거나 디스크에 기록하지 않습니다. 성공한 자동 교정의 단어 쌍과 누적 횟수만 로컬 통계로 저장합니다.
...
- `설정 → 통계`에서 누적 자동 교정과 단어별 횟수를 보고 언제든 통계만 초기화할 수 있습니다.
